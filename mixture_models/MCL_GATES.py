...
class MCL_GATES(nn.Module):
    def __init__(self, model_num, num_classes):
        super(MCL_GATES, self).__init__()
    ...
    def forward(self, input, target):
        ...
        # for idx in range(self.model_num):
            ...
            # self.gate_loss += F.binary_cross_entropy(confidence_detail[:, idx], (min_loss_idx == idx).squeeze(dim=1).float())

        self.gate_loss = criterion(confidence_detail, min_k_loss_idx[:, 0]).mean()
    ...
    def backward(self):
        self.loss = self.expert_mcl_loss + self.gate_loss
        self.loss.backward()
